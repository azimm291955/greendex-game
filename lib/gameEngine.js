...
function handleDraw(state) {
  ...
  if (side.deck.length === 0) {
    // deck out = lose
    const winner = sn === 'player' ? 'opponent' : 'player';
    const newState = { ...state, [sn]: side, phase: PHASE.GAME_OVER, winner };
    return log(newState, `📭 ${sn.toUpperCase()} has no cards to draw — ${winner.toUpperCase()} wins!`);
  }
