...
// ────────────────────────────────────────────────────────────
export const PHASE = {
  COIN_FLIP:      'coinFlip',      // opening coin flip decides who goes first
  SETUP:          'setup',
...
// Deal an opening hand that is guaranteed to contain at least one
// Stage-1 strain. Reshuffles the whole deck + hand and redraws until
// the constraint is satisfied (deck composition makes this fast).
function dealOpeningHand(deckDef) {
  let deck = shuffle(deckDef.map(c => ({ ...c })));
  let hand = deck.splice(0, 5);
  let safety = 0;
  while (!hand.some(c => c.kind === CARD_KIND.STRAIN && c.stage === 1) && safety < 50) {
...
export function buildInitialState() {
  const { deck: playerDeck, hand: playerHand } = dealOpeningHand(PLAYER_DECK_DEF);
  const { deck: oppDeck,    hand: oppHand    } = dealOpeningHand(OPPONENT_DECK_DEF);
  ...
  // Opponent sets itself up automatically: first Stage-1 strain as Active,
  // then up to 3 more Stage-1 strains on its bench.
  const oActiveIdx = opponent.hand.findIndex(c => c.kind === CARD_KIND.STRAIN && c.stage === 1);
  if (oActiveIdx !== -1) {
    const [card] = opponent.hand.splice(oActiveIdx, 1);
    opponent.active = createFieldCard(card);
    opponent.active.entryProtection = false;
  }
  while (opponent.bench.length < 3) {
    const idx = opponent.hand.findIndex(c => c.kind === CARD_KIND.STRAIN && c.stage === 1);
    if (idx === -1) break;
    const [card] = opponent.hand.splice(idx, 1);
    const fc = createFieldCard(card);
    fc.entryProtection = false;
    opponent.bench.push(fc);
  }

  // Player picks their own Active (and optional bench) during the SETUP phase.
  // The match opens with an interactive coin flip (PHASE.COIN_FLIP) that
  // decides who takes the first turn.
  return {
    phase: PHASE.COIN_FLIP,
    turnPhase: TURN_PHASE.DRAW,
    turnNumber: 1,
    player,
    opponent,
    log: ['🪙 Flip a coin to see who goes first!'],
    winner: null,
    coinResult: null,         // 'player' | 'opponent' once the flip resolves
    awaitingSetup: true,      // player still needs to choose their Active
    pendingPromotion: null,   // { returnPhase } when active is KO'd and bench exists
    pendingAutoEndTurn: false,// set after the player attacks → auto-ends the turn
...
export function gameReducer(state, action) {
  switch (action.type) {
    ...
    case 'RESOLVE_COIN_FLIP':   return handleResolveCoinFlip(state, action);
    case 'SETUP_PLACE_ACTIVE':  return handleSetupPlaceActive(state, action);
    case 'SETUP_PLACE_BENCH':   return handleSetupPlaceBench(state, action);
    case 'SETUP_RETURN_TO_HAND':return handleSetupReturnToHand(state, action);
    case 'START_BATTLE':        return handleStartBattle(state);
...
function checkKO(state, sideName) {
  ...
  if (updatedSide.bench.length > 0) {
    if (sideName === 'player') {
      // Remember which turn this KO happened on so we can resume it after the
      // player promotes a bench card (the KO may have occurred on the
      // opponent's turn). This prevents the bench-promotion step from being
      // skipped and forcing the player to replace from their hand instead.
      newState = {
        ...newState,
        phase: PHASE.SELECT_BENCH,
        pendingPromotion: { returnPhase: state.phase },
      };
    } else {
...
function checkWinConditions(state) {
  // During setup the player legitimately has no Active yet — don't
  // mistake that for an elimination loss.
  if (state.phase === PHASE.SETUP) return state;
...
// ── COIN FLIP: decide who takes the first turn ───────────────
function handleResolveCoinFlip(state, { result }) {
  if (state.phase !== PHASE.COIN_FLIP) return state;
  // result: 'player' | 'opponent' (computed by the UI from Math.random())
  const firstTurn = result === 'opponent' ? 'opponent' : 'player';

  let newState = { ...state, coinResult: firstTurn };

  if (firstTurn === 'opponent') {
    // Opponent won the flip and takes its opening turn before the player
    // chooses an Active. The player's Setup happens once that turn ends.
    newState = { ...newState, phase: PHASE.OPPONENT_TURN, turnPhase: TURN_PHASE.DRAW };
    return log(newState, '🪙 Opponent won the flip — they go first!');
  }

  // Player won the flip → proceed straight to Setup.
  newState = { ...newState, phase: PHASE.SETUP };
  return log(newState, '🪙 You won the flip — choose your Active strain to begin!');
}

// ── SETUP: player places their starting Active / bench ───────
function handleSetupPlaceActive(state, { cardIdx }) {
  if (state.phase !== PHASE.SETUP) return state;
  const side = cloneSide(state.player);
  if (side.active) return state; // already chosen
  const card = side.hand[cardIdx];
  if (!card || card.kind !== CARD_KIND.STRAIN || card.stage !== 1) return state;

  side.hand = side.hand.filter((_, i) => i !== cardIdx);
  const fc = createFieldCard(card);
  fc.entryProtection = false; // start-of-game placement
  side.active = fc;

  const newState = { ...state, player: side };
  return log(newState, `🌿 You set ${card.name} as your Active Strain!`);
}

function handleSetupPlaceBench(state, { cardIdx }) {
  if (state.phase !== PHASE.SETUP) return state;
  const side = cloneSide(state.player);
  if (!side.active) return state;       // must pick Active first
  if (side.bench.length >= 3) return state;
  const card = side.hand[cardIdx];
  if (!card || card.kind !== CARD_KIND.STRAIN || card.stage !== 1) return state;
  ...
  const fc = createFieldCard(card);
  fc.entryProtection = false;
  side.bench = [...side.bench, fc];

  const newState = { ...state, player: side };
  return log(newState, `🌱 You placed ${card.name} on your Bench.`);
}

function handleSetupReturnToHand(state, { from }) {
  // from: 'active' | bench index (number)
  if (state.phase !== PHASE.SETUP) return state;
  const side = cloneSide(state.player);
  ...
  if (from === 'active') {
    ...
    if (from < 0 || from >= side.bench.length) return state;
    card = side.bench[from];
    side.bench = side.bench.filter((_, i) => i !== from);
  } else {
    return state;
  }

  // The field card retains every original card-def field (createFieldCard
  // spreads ...cardDef), so it renders fine in hand; re-placement runs
  // createFieldCard again and regenerates a fresh instance.
  side.hand = [...side.hand, card];

  const newState = { ...state, player: side };
  return log(newState, `↩️ ${card.name} returned to your hand.`);
}

function handleStartBattle(state) {
  if (state.phase !== PHASE.SETUP) return state;
  if (!state.player.active) return state; // Active is required
  // Setup is complete; from here on the player takes their turn. (If the
  // opponent won the flip they've already had their opening turn.)
  let newState = {
    ...state,
  ...
  return log(newState, '⚔️ Battle Start! Your turn — Draw Phase.');
}
    ...
    case 'RECALL': {
      ...
      if (s.active && s.bench.length < 3) {
        ...
        if (!s.active && s.bench.length > 0) {
          if (sn === 'player') {
            newState = { ...newState, phase: PHASE.SELECT_BENCH, pendingPromotion: { returnPhase: PHASE.PLAYER_TURN } };
          } else {
...
export function handleAttack(state, { attackIdx }) {
  ...
  let newState = {
    ...
    // Attacking is always the last action of a turn. For the human player we
    // flag the turn to auto-end (handled with a short delay in the UI so the
    // hit animation can play). The AI ends its own turn via its action queue.
    pendingAutoEndTurn: sn === 'player',
...
function handleRetreat(state) {
  ...
  if (sn === 'player') {
    return { ...newState, phase: PHASE.SELECT_BENCH, pendingPromotion: { returnPhase: PHASE.PLAYER_TURN } };
  } else {
...
function handlePromote(state, { benchIdx }) {
  ...
  // Where to resume after promotion. If the KO happened on the opponent's
  // turn, we must hand control back to the opponent and properly finish its
  // turn (rather than jumping straight to the player's turn).
  const returnPhase = state.pendingPromotion?.returnPhase || PHASE.PLAYER_TURN;

  // Entry passives on promotion
  let newState = { ...state, player: side, phase: PHASE.PLAYER_TURN, pendingPromotion: null };
  ...
  if (returnPhase === PHASE.OPPONENT_TURN) {
    // Resume and complete the opponent's turn so the player gets a clean turn.
    newState = { ...newState, phase: PHASE.OPPONENT_TURN };
...
function handleEndTurn(state) {
  // Never end the turn while the player still owes a bench promotion — the
  // SELECT_BENCH step must complete first (otherwise the AI's queued END_TURN
  // would clobber it and force a replacement from the hand).
  if (state.phase === PHASE.SELECT_BENCH) return state;
  ...
  // If the opponent won the coin flip, it takes an opening turn *before* the
  // player has set up. When that turn ends, send the player into SETUP rather
  // than straight into their turn.
  const opponentOpeningTurn = sn === 'opponent' && state.awaitingSetup;

  const nextPhase = opponentOpeningTurn
    ? PHASE.SETUP
    : (sn === 'player' ? PHASE.OPPONENT_TURN : PHASE.PLAYER_TURN);
  // Don't advance the turn counter for the opponent's pre-setup opening turn.
  const nextTurnNumber = (sn === 'opponent' && !opponentOpeningTurn)
    ? state.turnNumber + 1
    : state.turnNumber;
  ...
  let newState = {
    ...
    pendingAutoEndTurn: false,
    shakingCard: null,
  };

  if (opponentOpeningTurn) {
    newState = log(newState, '🌱 Opponent is set. Now choose your Active strain!');
  } else {
